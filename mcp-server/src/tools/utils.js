...
export function executeTaskMasterCommand(
  ...
  try {
    log.info(
      `Executing hackmindai ${command} with args: ${JSON.stringify(
        args
      )} in directory: ${cwd}`
    );
    ...
    // Execute the command using the global hackmindai CLI or local script
    // Try the global CLI first
    let result = spawnSync("hackmindai", fullArgs, spawnOptions);

    // If global CLI is not available, try fallback to the local script
    if (result.error && result.error.code === "ENOENT") {
      log.info("Global hackmindai not found, falling back to local script");
    ...
    log.error(`Error executing hackmindai command: ${error.message}`);
    return {
      success: false,
      error: error.message,
    };
