...
function createDevScriptAction(commandName) {
  return (options, cmd) => {
    ...
    if (camelCaseFlags.length > 0) {
      ...
      console.error('\nExample: hackmindai parse-prd --num-tasks=5 instead of --numTasks=5\n');
      process.exit(1);
    }
...
program
  .name('hackmindai')
  .description('Claude HackMind CLIAI')
  .version(version)
...
// Show help if no command was provided (just 'hackmindai' with no args)
