...
2. **CLI Commands**  
   ...
   Run `hackmindai --help` or `node scripts/dev.js --help` to see detailed usage information.
...
hackmindai list --status=pending
...
hackmindai list --status=pending --with-subtasks
```
...
hackmindai update --from=4 --prompt="Refactor tasks from ID 4 onward to use Express instead of Fastify"

# Update all tasks (default from=1)
hackmindai update --prompt="Add authentication to all relevant tasks"

# Specify a different tasks file
hackmindai update --file=custom-tasks.json --from=5 --prompt="Change database from MongoDB to PostgreSQL"
```
...
hackmindai set-status --id=4 --status=pending

# Mark a specific subtask as done
hackmindai set-status --id=3.1 --status=done

# Mark multiple tasks at once
hackmindai set-status --id=1,2,3 --status=done
```
...
hackmindai expand --id=3

# Expand a specific task with 5 subtasks
hackmindai expand --id=3 --num=5

# Expand a task with additional context
hackmindai expand --id=3 --prompt="Focus on security aspects"
...
hackmindai expand --id=3 --research
...
hackmindai clear-subtasks --id=3
...
# Specify a different tasks file
hackmindai validate-dependencies --file=custom-tasks.json
```
...
# Specify a different tasks file
hackmindai fix-dependencies --file=custom-tasks.json
```
...
hackmindai analyze-complexity --output=custom-report.json
...
hackmindai expand --id=8 --num=5 --prompt="Custom prompt"
```
  ...
  "complexityAnalysis": [
    {
      ...
      "expansionCommand": "hackmindai expand --id=8 --num=6 --prompt=\"Create subtasks...\" --research"
    },
...
# Specify a different tasks file
hackmindai next --file=custom-tasks.json
```
...
hackmindai show --id=1

# Show details for a subtask
hackmindai show --id=1.2

# Specify a different tasks file
hackmindai show 3 --file=custom-tasks.json
```
