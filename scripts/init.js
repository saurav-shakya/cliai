...
console.log('Starting hackmindai-ai...');
...
const coolGradient = gradient(['#FF671F', '#FFFFFF', '#046A38']);
const warmGradient = gradient(['#FF9933', '#FFFFFF', '#138808']);
...
function displayBanner() {
  console.clear();
  const bannerText = figlet.textSync('HackMind   CLIAI', {
    font: 'Standard',
  ...
  console.log(boxen(chalk.white(`${chalk.bold('Initializing')} Your Idea Create My Fully Vision`), {
    padding: 1,
...
function copyTemplateFile(templateName, targetPath, replacements = {}) {
  ...
  switch(templateName) {
    ...
    case 'self_improve.mdc':
      ...
      break;
    case 'README-hackmindai.md':
      sourcePath = path.join(__dirname, '..', 'README-hackmindai.md');
      break;
    case 'windsurfrules':
  ...
  if (fs.existsSync(targetPath)) {
    ...
    if (filename === 'README.md') {
      ...
      const taskMasterReadmePath = path.join(path.dirname(targetPath), 'README-hackmindai.md');
...
function createProjectStructure(projectName, projectDescription, projectVersion, authorName, skipInstall) {
  ...
  copyTemplateFile('README-hackmindai.md', path.join(targetDir, 'README.md'), replacements);
  ...
  console.log(boxen(
    ...
    chalk.white('   └─ ') + chalk.dim('You can also run ') + chalk.cyan('hackmindai parse-prd <your-prd-file.txt>') + '\n' +
  ...
  try {
    ...
    if (options.yes || (options.name && options.description)) {
      ...
      await initializeProject({
        projectName: options.name || 'hackmindai-project',
