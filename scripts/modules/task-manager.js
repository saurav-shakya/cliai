...
async function parsePRD(prdPath, tasksPath, numTasks) {
  try {
    ...
    console.log(boxen(
      ...
      `${chalk.cyan('2.')} Run ${chalk.yellow('hackmindai expand --id=<id>')} to break down a task into subtasks`,
  ...
  if (taskIdInput.includes('.')) {
    ...
    if (newStatus.toLowerCase() === 'done' || newStatus.toLowerCase() === 'completed') {
      ...
      if (allSubtasksDone && parentTask.status !== 'done' && parentTask.status !== 'completed') {
        ...
        console.log(chalk.yellow(`Consider updating the parent task status with: hackmindai set-status --id=${parentId} --status=done`));
      }
    }
  } else {
    // Handle regular task
  ...
  try {
    ...
    if (nextTask) {
      ...
      console.log(boxen(
        ...
        subtasksSection + '\n\n' +
        `${chalk.cyan('Start working:')} ${chalk.yellow(`hackmindai set-status --id=${nextTask.id} --status=in-progress`)}\n` +
        `${chalk.cyan('View details:')} ${chalk.yellow(`hackmindai show ${nextTask.id}`)}`,
        { 
    ...
    console.log(boxen(
      ...
      `${chalk.cyan('1.')} Run ${chalk.yellow('hackmindai next')} to see what to work on next\n` +
      `${chalk.cyan('2.')} Run ${chalk.yellow('hackmindai expand --id=<id>')} to break down a task into subtasks\n` +
      `${chalk.cyan('3.')} Run ${chalk.yellow('hackmindai set-status --id=<id> --status=done')} to mark a task as complete`,
...
async function expandTask(taskId, numSubtasks = CONFIG.defaultSubtasks, useResearch = false, additionalContext = '') {
  try {
    ...
    console.log(boxen(
      chalk.white.bold('Next Steps:') + '\n\n' +
      `${chalk.cyan('1.')} Run ${chalk.yellow(`hackmindai show ${taskId}`)} to see the full task with subtasks\n` +
      `${chalk.cyan('2.')} Start working on subtask: ${chalk.yellow(`hackmindai set-status --id=${taskId}.1 --status=in-progress`)}\n` +
      `${chalk.cyan('3.')} Mark subtask as done: ${chalk.yellow(`hackmindai set-status --id=${taskId}.1 --status=done`)}`,
      { padding: 1, borderColor: 'cyan', borderStyle: 'round', margin: { top: 1 } }
    ));
  } catch (error) {
...
async function expandAllTasks(numSubtasks = CONFIG.defaultSubtasks, useResearch = false, additionalContext = '', forceFlag = false) {
  try {
    ...
    console.log(boxen(
      chalk.white.bold('Next Steps:') + '\n\n' +
      `${chalk.cyan('1.')} Run ${chalk.yellow('hackmindai list --with-subtasks')} to see all tasks with subtasks\n` +
      `${chalk.cyan('2.')} Run ${chalk.yellow('hackmindai next')} to see what to work on next`,
      { padding: 1, borderColor: 'cyan', borderStyle: 'round', margin: { top: 1 } }
    ));
  } catch (error) {
...
function clearSubtasks(tasksPath, taskIds) {
  ...
  if (clearedCount > 0) {
    ...
    console.log(boxen(
      chalk.white.bold('Next Steps:') + '\n\n' +
      `${chalk.cyan('1.')} Run ${chalk.yellow('hackmindai expand --id=<id>')} to generate new subtasks\n` +
      `${chalk.cyan('2.')} Run ${chalk.yellow('hackmindai list --with-subtasks')} to verify changes`,
      { padding: 1, borderColor: 'cyan', borderStyle: 'round', margin: { top: 1 } }
    ));
...
async function addTask(tasksPath, prompt, dependencies = [], priority = 'medium') {
  ...
  try {
    ...
    console.log(boxen(
      chalk.white.bold('Next Steps:') + '\n\n' +
      `${chalk.cyan('1.')} Run ${chalk.yellow('hackmindai generate')} to update task files\n` +
      `${chalk.cyan('2.')} Run ${chalk.yellow('hackmindai expand --id=' + newTaskId)} to break it down into subtasks\n` +
      `${chalk.cyan('3.')} Run ${chalk.yellow('hackmindai list --with-subtasks')} to see all tasks`,
      { padding: 1, borderColor: 'cyan', borderStyle: 'round', margin: { top: 1 } }
    ));
