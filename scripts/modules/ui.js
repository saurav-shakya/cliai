...
function displayBanner() {
  ...
  console.log(chalk.dim('by ') + chalk.cyan.underline('https://hackmind.in/cliai'));
...
async function displayNextTask(tasksPath) {
  ...
  if (nextTask.subtasks && nextTask.subtasks.length > 0) {
    ...
    console.log(boxen(
      ...
      chalk.white(`Run: ${chalk.cyan(`hackmindai expand --id=${nextTask.id}`)}`),
  ...
  console.log(boxen(
    ...
    `${chalk.cyan('1.')} Mark as in-progress: ${chalk.yellow(`hackmindai set-status --id=${nextTask.id} --status=in-progress`)}\n` +
    `${chalk.cyan('2.')} Mark as done when completed: ${chalk.yellow(`hackmindai set-status --id=${nextTask.id} --status=done`)}\n` +
    (nextTask.subtasks && nextTask.subtasks.length > 0 
      ? `${chalk.cyan('3.')} Update subtask status: ${chalk.yellow(`hackmindai set-status --id=${nextTask.id}.1 --status=done`)}`
      : `${chalk.cyan('3.')} Break down into subtasks: ${chalk.yellow(`hackmindai expand --id=${nextTask.id}`)}`),
...
async function displayTaskById(tasksPath, taskId) {
  ...
  if (task.isSubtask || task.parentTask) {
    ...
    console.log(boxen(
      chalk.white.bold('Suggested Actions:') + '\n' +
      `${chalk.cyan('1.')} Mark as in-progress: ${chalk.yellow(`hackmindai set-status --id=${task.parentTask.id}.${task.id} --status=in-progress`)}\n` +
      `${chalk.cyan('2.')} Mark as done when completed: ${chalk.yellow(`hackmindai set-status --id=${task.parentTask.id}.${task.id} --status=done`)}\n` +
      `${chalk.cyan('3.')} View parent task: ${chalk.yellow(`hackmindai show --id=${task.parentTask.id}`)}`,
      { padding: { top: 0, bottom: 0, left: 1, right: 1 }, borderColor: 'green', borderStyle: 'round', margin: { top: 1 } }
    ));
  ...
  if (task.subtasks && task.subtasks.length > 0) {
    ...
    console.log(boxen(
      chalk.yellow('No subtasks found. Consider breaking down this task:') + '\n' +
      chalk.white(`Run: ${chalk.cyan(`hackmindai expand --id=${task.id}`)}`),
      { padding: { top: 0, bottom: 0, left: 1, right: 1 }, borderColor: 'yellow', borderStyle: 'round', margin: { top: 1, bottom: 0 } }
    ));
  }
  ...
  console.log(boxen(
    chalk.white.bold('Suggested Actions:') + '\n' +
    `${chalk.cyan('1.')} Mark as in-progress: ${chalk.yellow(`hackmindai set-status --id=${task.id} --status=in-progress`)}\n` +
    `${chalk.cyan('2.')} Mark as done when completed: ${chalk.yellow(`hackmindai set-status --id=${task.id} --status=done`)}\n` +
    (task.subtasks && task.subtasks.length > 0 
      ? `${chalk.cyan('3.')} Update subtask status: ${chalk.yellow(`hackmindai set-status --id=${task.id}.1 --status=done`)}`
      : `${chalk.cyan('3.')} Break down into subtasks: ${chalk.yellow(`hackmindai expand --id=${task.id}`)}`),
    { padding: { top: 0, bottom: 0, left: 1, right: 1 }, borderColor: 'green', borderStyle: 'round', margin: { top: 1 } }
  ));
}
...
async function displayComplexityReport(reportPath) {
  ...
  tasksNeedingExpansion.forEach(task => {
    const expansionCommand = `hackmindai expand --id=${task.taskId} --num=${task.recommendedSubtasks}${task.expansionPrompt ? ` --prompt="${task.expansionPrompt}"` : ''}`;
  ...
  console.log(boxen(
    chalk.white.bold('Suggested Actions:') + '\n\n' +
    `${chalk.cyan('1.')} Expand all complex tasks: ${chalk.yellow(`hackmindai expand --all`)}\n` +
    `${chalk.cyan('2.')} Expand a specific task: ${chalk.yellow(`hackmindai expand --id=<id>`)}\n` +
    `${chalk.cyan('3.')} Regenerate with research: ${chalk.yellow(`hackmindai analyze-complexity --research`)}`,
