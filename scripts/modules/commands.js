...
function registerCommands(programInstance) {
    ...
    .action(async (file, options) => {
      ...
      if (!inputFile) {
        ...
        console.log(boxen(
          ...
          chalk.cyan('Usage:') + '\n' +
          `  hackmindai parse-prd <prd-file.txt> [options]\n\n` +
          chalk.cyan('Options:') + '\n' +
          '  -i, --input <file>       Path to the PRD file (alternative to positional argument)\n' +
          '  -o, --output <file>      Output file path (default: "tasks/tasks.json")\n' +
          '  -n, --num-tasks <number> Number of tasks to generate (default: 10)\n\n' +
          chalk.cyan('Example:') + '\n' +
          '  hackmindai parse-prd requirements.txt --num-tasks 15\n' +
          '  hackmindai parse-prd --input=requirements.txt\n\n' +
      ...
      try {
        if (existingTaskId) {
          ...
          console.log(boxen(
            ...
            chalk.cyan(`1. Run ${chalk.yellow(`hackmindai show ${parentId}`)} to see the parent task with all subtasks`) + '\n' +
            chalk.cyan(`2. Run ${chalk.yellow(`hackmindai set-status --id=${parentId}.${subtask.id} --status=in-progress`)} to start working on it`),
          ...
          console.log(boxen(
            ...
            chalk.yellow(`  hackmindai add-subtask --parent=5 --task-id=8`) + '\n\n' +
            chalk.white('Create new subtask:') + '\n' +
            chalk.yellow(`  hackmindai add-subtask --parent=5 --title="Implement login UI" --description="Create the login form"`) + '\n\n',
            { padding: 1, borderColor: 'blue', borderStyle: 'round' }
          ));
          process.exit(1);
        }
      ...
      try {
        ...
        if (convertToTask && result) {
          ...
          console.log(boxen(
            ...
            chalk.white.bold('Next Steps:') + '\n' +
            chalk.cyan(`1. Run ${chalk.yellow(`hackmindai show ${result.id}`)} to see details of the new task`) + '\n' +
            chalk.cyan(`2. Run ${chalk.yellow(`hackmindai set-status --id=${result.id} --status=in-progress`)} to start working on it`),
            { padding: 1, borderColor: 'green', borderStyle: 'round', margin: { top: 1 } }
          ));
        } else {
  ...
  programInstance
    ...
    .action(() => {
      console.log(chalk.yellow('The init command must be run as a standalone command: hackmindai init'));
      console.log(chalk.cyan('Example usage:'));
      console.log(chalk.white('  hackmindai init -n "My Project" -d "Project description"'));
      console.log(chalk.white('  hackmindai init -my_name "My Project" -my_description "Project description"'));
      console.log(chalk.white('  hackmindai init -y'));
      process.exit(0);
    });
